...
iptables -A INPUT -s 0.0.0.0/0 -d 10.0.0.104/32 -p tcp --sport 1024:65535 --dport 22 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A OUTPUT -s 10.0.0.104/32 -d 0.0.0.0/0 -p tcp --sport 22 --dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT
...
iptables -t nat -A PREROUTING -p tcp --dport 80 -d 10.0.0.104 -i eth0 -j DNAT --to-destination 10.0.0.12
iptables -t nat -A POSTROUTING -p tcp --sport 80 -s 10.0.0.12 -o eth0 -j SNAT --to-source 10.0.0.104
# Reglas para el proxy
iptables -t nat -A POSTROUTING -p tcp --dport 80 -s 10.0.0.26 -o eth0 -j SNAT --to-source 10.0.0.104
iptables -t nat -A POSTROUTING -p tcp --dport 443 -s 10.0.0.26 -o eth0 -j SNAT --to-source 10.0.0.104
# Reglas para el servidor privado
iptables -t nat -A PREROUTING -p udp --dport 51820  -d 10.0.0.104 -i eth0 -j DNAT --to-destination 10.0.0.39
iptables -t nat -A POSTROUTING -p udp --sport 51820 -s 10.0.0.39 -o eth0 -j SNAT --to-source 10.0.0.104
...
# Reglas para el servidor privado
iptables -A FORWARD -p udp -s 0.0.0.0/0 -d 10.0.0.39/32 --sport 1024:65535 --dport 51820 -j ACCEPT 
iptables -A FORWARD -p udp -s 10.0.0.39 -d 0.0.0.0/0 --sport 51820 --dport 1024:65535 -j ACCEPT 
